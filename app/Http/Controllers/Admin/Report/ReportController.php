...
class ReportController extends Controller
{
    ...
    public function saleReport(Request $request)
    {
        ...
        abort_if(!auth()->user()->can(abilities: 'view_sales'), 403);
        ...
        $orders = Sale::whereBetween('created_at', [$start_date, $end_date])->with('customer')->get();
        ...
        return view('admin.reports.sale-report', $data);
    }
    public function saleSummery(Request $request)
    {

        abort_if(!auth()->user()->can('view_sales'), 403);
        // Get user input or set default values
        $start_date_input = $request->input('start_date', Carbon::today()->subDays(29)->format('Y-m-d'));
        $end_date_input = $request->input('end_date', Carbon::today()->format('Y-m-d'));
        ...
        $end_date = $end_date->endOfDay();
        // Retrieve orders within the date range
        $orders = Sale::whereBetween('created_at', [$start_date, $end_date])->get();
        ...
        return view('admin.reports.sale-summery', $data);
    }
